...
**Profundidad**: La profundidad de cada contig se calcula mediante el mapeo de las lecturas al ensamble. Este paso permite evaluar la calidad del ensable y es necesario para hacer la reconstrucción de genomas ya que, como veremos más adelante, es uno de los parámetros que toman en cuenta los "bineadores". 
...
```{bash, eval=False}
conda avtivate maxbin
```

Explora las opciones y luego ahora si, a calcular bins. 

```{bash run MaxBin2, eval=FALSE}
run_MaxBin.pl -contig data/htn.fasta -out results/ -abund data/htn-depth.txt
```
...
El resto de argumentos que vamos a usar se refieren a:
 ...
 - m: Tamaño mínimo de un contig para binning (debe ser >=1500). Usualmente usamos 2000.
...
merge_cutup_clustering.py results/concot_clustering_gt3000.csv > results/merged-htn-gt3000.csv
```
...
DAS_Tool -i htn_concot.scaffolds2bin.tsv,htn.scaffolds2bin.tsv -l concoct,metabat -c htn.fasta -o htn_bins --debug -t 1 --write_bins 1 --search_engine blast >DAStools-log_htn 
```

### CheckM

```{bash, eval=FALSE}
#/home/mirna/00.Programs/miniconda3/envs/py27/bin/.checkm data setRoot /home/mirna/00.Programs/miniconda3/envs/py27/check_db
conda activate py27
checkm  lineage_wf -t 20 -x fa DAStools-log_htn CheckM_DAS_Tool -f CheckM-DAS_Tool_bins.txt
```
